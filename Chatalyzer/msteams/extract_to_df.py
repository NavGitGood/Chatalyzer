...
def getRootMessages(teamid:None, channelid:None):
    file = './Chatalyzer/msteams/root-msgs.json'    # to be replaced with actual api call
    with open(file) as json_file:
        jsonData = json.load(json_file)
    jsonList = list(jsonData["value"])
    rootDF = jsonParser(jsonList)
    # df = removeDuplicates(rootDF[['ID']])
    getReplies(list(removeDuplicates(rootDF[['ID']])['ID']))
    return rootDF
...
def getReplies(messageIds):
    ...
    for messageId in messageIds:
        if(messageId=='1548100551644'):
            ...
            file = f'./Chatalyzer/msteams/reply_{messageId}.json'  # to be replaced with actual api call and handle response with no replies
            with open(file) as json_file:
                jsonData = json.load(json_file)
            jsonList = list(jsonData["value"])
