...
def userPlotter(username):
    fig = plt.figure(figsize=(15,15))
    ax = fig.add_subplot()
    df = discreetSentimentForIndividual(username)
    ...
    fig.savefig('ax_figure.png', bbox_inches=extent.expanded(1.5,1.5))
    plt.show()
...
def groupPlotter():
    ...
    fig.savefig('ax1_figure.png', bbox_inches=extent.expanded(1.0,3.0))
    fig.delaxes(ax1)


    df2 = wordCountByAuthor(5)
    ...
    fig.savefig('ax2_figure.png', bbox_inches=extent.expanded(1.0,3.0))
    fig.delaxes(ax2)


    df3 = letterCountByAuthor(5)
    ...
    fig.savefig('ax3_figure.png', bbox_inches=extent.expanded(1.0,3.0))
    fig.delaxes(ax3)


    df4 = sentimentPolarityCountByAuthor()
    ...
    # df4.plot.scatter(x='sentiment-polarity', y='Message', ax=ax4, c=df4['Author'].apply(lambda x: color_map[x]))
    ...
    fig.savefig('ax4_figure.png', bbox_inches=extent.expanded(1.15,3.0))
    plt.tight_layout()
    plt.show()

def getTopAuthorsByMessages(df):
    df.plot.pie()
    plt.title('Authors by Messages')
    plt.show()

def getTopAuthorsByWords(df):
    df.plot.barh()
    plt.xlabel(authors_by_words.xlabel)
    plt.ylabel(authors_by_words.ylabel)
    plt.title('Authors by Words')
    # plt.gcf().canvas.set_window_title('Analysis Visualization')
    plt.show()

def getTopAuthorsByLetters(df):
    df.plot.barh()
    plt.xlabel(authors_by_letters.xlabel)
    plt.ylabel(authors_by_letters.ylabel)
